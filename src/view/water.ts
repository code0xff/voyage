...
const vertexShader = /* glsl */ `
  ...
  void main() {
    ...
    float fade = 1.0 - smoothstep(0.28, 0.49, edge);
...
const fragmentShader = /* glsl */ `
  ...
  void main() {
    ...
    col += vec3(1.0, 0.98, 0.92) * pow(max(dot(n, h), 0.0), 90.0) * 0.5;
    ...
    // steep, so require both.
    if (vAmp > 0.001) {
      float crest = smoothstep(0.72, 0.99, vHeight / vAmp);
      float steep = smoothstep(0.22, 0.55, vSteep);
      col = mix(col, vec3(0.86, 0.91, 0.95), crest * steep * uWhitecap);
    }
...
export interface Water {
  ...
  update(waves: WaveField, simX: number, simY: number, tws: number): void;
  dispose(): void;
}
...
export function createWater(fogColor: number, fogNear: number, fogFar: number): Water {
  ...
  const uniforms = {
    ...
    uSun: { value: new THREE.Vector3(-60, 90, 40).normalize() },
